...
async fn test_idempotency_multiple_runs_on_same_library() {
    ...
    assert_eq!(movies2.len(), 2, "Second scan should find only 2 movies without done markers");
    ...
    assert_eq!(movies3.len(), 2, "Third scan should find same 2 movies (idempotent)");
    ...
    assert_eq!(movies4.len(), 0, "Fourth scan should find no movies (all processed)");
}
...
async fn test_idempotency_interruption_and_resumption() {
    ...
    assert_eq!(movies_resume.len(), 2, "Resume should find 2 unprocessed movies");
    ...
    assert!(temp_base.exists(), "Temp directories should exist before cleanup");
    ...
    assert_eq!(movies_final.len(), 1, "Final scan should find only 1 unprocessed movie");
...
async fn test_idempotency_force_flag_behavior() {
    ...
    let movies_no_force = scanner_no_force.scan().expect("Scan without force should succeed");
    assert_eq!(movies_no_force.len(), 0, "Without force flag, should skip all movies with done markers");
    
    // Scan with force flag - should find all movies
    let scanner_force = Scanner::new(root.clone(), true);
    let movies_force = scanner_force.scan().expect("Scan with force should succeed");
    assert_eq!(movies_force.len(), 3, "With force flag, should find all movies");
    
    // Verify all movies have done markers but are still included
    for movie in &movies_force {
        assert!(movie.has_done_marker, "Movie should have done marker: {}", movie.title);
    }
    
    // Multiple force scans should be idempotent
    let scanner_force2 = Scanner::new(root.clone(), true);
    let movies_force2 = scanner_force2.scan().expect("Second force scan should succeed");
    assert_eq!(movies_force2.len(), 3, "Multiple force scans should be idempotent");
}
...
async fn test_idempotency_partial_library_processing() {
    ...
    let scanner3 = Scanner::new(root.clone(), false);
    let movies3 = scanner3.scan().expect("Third scan should succeed");
    assert_eq!(movies3.len(), 4, "Should find 4 unprocessed movies (1 old + 3 new)");
    ...
    let movies5 = scanner5.scan().expect("Scan after adding new movie should succeed");
...
async fn test_idempotency_invalid_done_markers() {
    ...
    // Empty file
    fs::write(movie3_dir.join("done.ext"), "")
        .expect("Failed to write empty done marker");
    ...
    let scanner = Scanner::new(root.clone(), false);
    let movies = scanner.scan().expect("Scan should succeed");
    assert_eq!(movies.len(), 3, "Should find 3 movies with invalid or missing done markers");
    
    // Verify correct movies found
    let titles: Vec<&str> = movies.iter().map(|m| m.title.as_str()).collect();
    assert!(titles.contains(&"Movie 2"), "Should find Movie 2 (invalid JSON)");
    assert!(titles.contains(&"Movie 3"), "Should find Movie 3 (empty file)");
    assert!(titles.contains(&"Movie 4"), "Should find Movie 4 (no marker)");
    assert!(!titles.contains(&"Movie 1"), "Should not find Movie 1 (valid marker)");
}
